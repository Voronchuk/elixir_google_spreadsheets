defmodule GSS.Spreadsheet do
  ...
  The `:end_column_index` option is required; every other option has a default.
  Returns `{:error, %GSS.InvalidInput{}}` (without touching the server) when
  `row_count` is not an integer or `:end_column_index` is missing or not an integer.

  ## Options

  * `:sheet_id` - ID of the sheet to sort (default: the worksheet's `sheet_id`, or 0 when unbound)
  ...
  * `:sort_order` - "ASCENDING" or "DESCENDING" (default: "ASCENDING")
  """
  @spec sort(pid, integer(), Keyword.t()) :: :ok | {:error, Exception.t()}
  def sort(pid, row_count, options \\ [])

  def sort(pid, row_count, options) when is_integer(row_count) and is_list(options) do
    case Keyword.get(options, :end_column_index) do
      end_column_index when is_integer(end_column_index) ->
        gen_server_call(pid, {:sort, row_count, options}, options)

      _ ->
        {:error,
         %GSS.InvalidInput{message: "sort/3 requires an integer :end_column_index option"}}
    end
  end

  def sort(_pid, _row_count, _options),
    do: {:error, %GSS.InvalidInput{message: "sort/3 requires an integer row_count"}}
  ...
  def handle_call(
        {:sort, row_count, options},
        _from,
        %{spreadsheet_id: spreadsheet_id, sheet_id: state_sheet_id} = state
      ) do
    url_suffix = "#{spreadsheet_id}:batchUpdate"

    sheet_id = Keyword.get(options, :sheet_id, state_sheet_id || 0)
  ...
  # Build the Authorization header, converting a missing-auth-config situation
  # into an ordinary `{:error, exception}` result instead of letting
  # `GSS.Auth.token!/0` raise and crash the GenServer. Only `GSS.MissingAuthConfig`
  # is rescued; every other exception propagates untouched.
  @spec auth_headers() :: {:ok, %{binary() => binary()}} | {:error, Exception.t()}
  defp auth_headers do
    {:ok, %{"Authorization" => "Bearer #{GSS.Auth.token!()}"}}
  rescue
    exception in GSS.MissingAuthConfig -> {:error, exception}
  end
